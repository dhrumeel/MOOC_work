...
class Graph:
    ...
    def read_from(self, filename):
        ...
        File format: First line "<number of nodes>  <number of edges>"
        ...
        assert self.num_vertices() == expected_num_nodes
    ...
    def n_cluster(self, n):
        """
        Partially runs kruskal's MST algorithm until a forest with n disjoint 
        components (clusters) is achieved.
        This guarantees that the clustering maximizes the distance between the
        two closest clusters.
        Returns the disjoint forest graph and the minimum spacing between clusters.
        """
        forest = Graph()
        map(lambda v: forest.add_vertex(v), self._vertices)
        uf = UnionFind()
        if n > self.num_vertices():
            raise ValueError("n is greater than the number of vertices")
        # Initialize union-find datastructure
        map(lambda v: uf.add_singleton(v), self._vertices)
        edge_iter = iter(sorted(self._edges, key=lambda e: e.cost))
        while uf.num_disjoint_sets > n:
            edge = next(edge_iter)
            (cost, vertex1, vertex2) = edge
            if uf.find_root(vertex1) == uf.find_root(vertex2):
                continue
            forest.add_edge(cost, vertex1, vertex2)
            uf.union(vertex1, vertex2)
        
        # Find the cheapest edge that doesn't result in a cycle
        min_spacing = None
        for edge in edge_iter:
            cost, vertex1, vertex2 = edge
            if uf.find_root(vertex1) != uf.find_root(vertex2):
                min_spacing = edge.cost
                break
        
        if min_spacing is None:
            raise ValueError("Graph seems to be disconnected")
        return (min_spacing, forest)
